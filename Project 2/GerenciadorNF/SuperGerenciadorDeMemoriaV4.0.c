...
*                                              *
*Trabalho 02 - Gerenciador de Memoria Next  Fit*
*                                              *
...
/***********************************************************************
*                                                                      *
*  $FC Função: ReadyToMemory -> NEXT FIT                               *
*                                                                      *
*                                                                      *
***********************************************************************/
void ReadyToMemory(ListOfProcesses *Lprocesses, Memory *Memory){// NEXT FIT
	int partAux;
	int indexMaior;
	int i;
	int var = 0;

	if (Memory->LMemory->quantity == 0){
		indexMaior = 0;
	}
	else if (Memory->LMemory->quantity > 0){
		indexMaior = BuscaMaiorOrdem(Memory);
	}

	for (i = indexMaior; i < 5; i++){
		if (Memory->LMemory->proc[i].order == -1 && Lprocesses->proc[0].Memory <= Memory->LMemory->proc[i].partition){
